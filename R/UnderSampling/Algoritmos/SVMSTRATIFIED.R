...
dfISCX<- read.csv(file="/home/latin/R/ISCX2017/MachineLearningCVE/export_dataframe5PercentCleanedPCA.csv",stringsAsFactors = FALSE)
...
for (split in 1:length(splits)) {
  ...
  for(ii in 1:runs ){
    ...
    model <- svm(data_train$X25 ~ ., data = data_train[,1:25],kernel = "radial")
...
#Salvando em arquivo
save(acc,f11,recalll,tim,file="svmStratified5Percent30Runs")
